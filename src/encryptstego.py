...
def open_encode_window():
    ...
    if not encode_opened:
        ...
        encode_window.title("Image Steganograpy by group - 07")
        # Setting the size of the encode window
        encode_window.geometry('1200x800')
        ...
        raw_image_label = Label(encode_window, text="Select Raw Image", height=34, width=90, relief="solid",
                                bg="#FFFFFF")
        ...
        text_to_encode_label.place(x=750, y=20)
        ...
        text_to_encode.place(x=750, y=51)
        ...
        pass_to_encode_label.place(x=750, y=262)
        ...
        pass_to_encode.place(x=750, y=293)
        ...
        browse_image_btn.place(x=120, y=570)
        ...
        encode_image_btn.place(x=850, y=380)
...
def open_decode_window():
    ...
    if not decode_opened:
        ...
        decode_window.geometry('1200x800')
        ...
        # Label to display image after the image is selected from the file window
        stego_image_label = Label(decode_window, text="Select Stego Image", height=34, width=90, relief="solid",
                                  bg="#FFFFFF")
        # Placing the label on the window statically
        ...
        text_to_decode_label.place(x=750, y=20)
        ...
        text_to_decode.place(x=750, y=51)
        ...
        pass_to_decode_label.place(x=750, y=262)
        ...
        pass_to_decode.place(x=750, y=293)
        ...
        browse_stego_btn.place(x=120, y=570)
        ...
        decode_stego_btn.place(x=850, y=380)
...
def browse_image(image_frame):
    ...
    #tkinter.filedialog.askopenfilename(filetypes = ([('png', '*.png'),('jpeg', '*.jpeg'),('jpg', '*.jpg'),('All Files', '*.*')]))
    ...
    selected_image=selected_image.resize((600,500))
    selected_image = ImageTk.PhotoImage(selected_image)
    # Setting the label configurations to display the image on the label
    image_frame.config(image=selected_image, height=510, width=600)
    image_frame.grid(x=0,y=0)
...
def help_menu():
    ...
    if not help_opened:
        ...
        text_to_decode_label = Label(help_window,
                                     text="This is an Image Steganography tool "
...
window.title("Image Steganography by group - 07")
...
title_label = Label(window, text="░▒▓ Hiding Confidential Message Using Image Steganogarphy ▓▒░")
# Packing the label on the window
title_label.pack()
# Setting font configurations for the label - font family and font size
title_label.config(font=("Open Sans", 20,'bold'))
title_label.place(x=55,y=50)
...
encode_btn.pack(side=LEFT, padx=200)
...
# Packing the button on the window
decode_btn.pack(side=LEFT, padx=50)

# Initializing footer name label
footer_label = Label(window, text="Group - 07")
# Packing the label on the window
...
menu = Menu(window)
# Adding help menu
menu.add_command(label="Want to know about steganography?", command=help_menu)
